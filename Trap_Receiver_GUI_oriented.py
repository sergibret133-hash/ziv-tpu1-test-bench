...
class Trap_Receiver_GUI_oriented:
    ...
    def start_trap_listener(self, listen_ip="0.0.0.0", listen_port=162, mib_dirs_string=""):
        ...
        # Limpiamos todo y preparamos el entorno MIB
        ...
        # La única tarea de este método es lanzar el hilo que hará todo el trabajo.
        self._listener_thread = threading.Thread(
            target=self._run_dispatcher_loop,
            args=(listen_ip, int(listen_port)),
            daemon=True
        )
    ...
    def _run_dispatcher_loop(self, listen_ip, listen_port):
        # Este método es ahora 100% autónomo y se ejecuta en su propio hilo.
        
        # 1. Crea y establece el bucle de eventos para ESTE hilo.
        asyncio.set_event_loop(asyncio.new_event_loop())

        # 2. Crea el dispatcher DENTRO de este hilo.
        self._transport_dispatcher = AsyncioDispatcher()
        self._transport_dispatcher.register_recv_callback(self._callback)
        
        try:
            # 3. Registra el transporte DENTRO de este hilo.
            self._transport_dispatcher.register_transport(
                udp.DOMAIN_NAME, udp.UdpAsyncioTransport().open_server_mode((listen_ip, listen_port))
            )
            logger.info(f"Listener de traps SNMP (IPv4) iniciado en {listen_ip}:{listen_port}")
        except Exception as e:
            logger.error(f"Error al registrar transporte UDP/IPv4 en {listen_ip}:{listen_port}: {e}")
            return

        # 4. Ejecuta el bucle del dispatcher.
