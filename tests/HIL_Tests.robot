...
Resource    start_browser.robot
Resource    Chronological_Register.robot
Resource    Test2_CommandAssign.robot
...
# **************************************************************
    FOR     ${i}    IN RANGE    1    ${NUM_PULSES} + 1
        ...
        ${new_traps_A}=    Get Traps Since Index    ${start_index_A}    A
        ${new_traps_B}=    Get Traps Since Index    ${start_index_B}    B
    ...
    ${csv_path}=    Generate Performance Report    ${rpi_logs}    ${ALL_TRAPS_A}    ${ALL_TRAPS_B}
