...
Check if Telep available and Open Section
    ...
    ${Telep_name_to_config}=    Set Variable    IDTU
...
#     Sleep    2s
# 2 Mbps frame channels for Teleprotection Assignment
#     2 Mbps frame channels for Teleprotection    1
# Programm
#     Program Old    sCOM
...
Assign Decision Threshold
    ...
    [Documentation]    Assigns the decision threshold for the IOCT commands.
    ...
    Input Text By Pressing Keys    ${threshold_locator}    ${threshold}
Assign Window Length
    [Arguments]    ${window_length}    ${command_number}
    [Documentation]    Assigns the window length for the IOCT commands.
    ...
    Input Text By Pressing Keys    ${window_length_locator}    ${window_length}
...
Autoconfig Predefine Tx_commands
    ...
    FOR    ${command}    IN RANGE    1    ${Max_Tx_Commands}
        Assign IOCT Command Type    ${command}    ${Command Type}
    END
...
Clock Type Selection
    ...
    [Documentation]    This keyword configures the clock type in the IOCS section. '0' for Internal, '1' for Recovered
    ...
    ${is_in_range}    Validate Number In Range - int    ${clock_type}    0    1
    Run Keyword If    not ${is_in_range}    Fail    Invalid clock type. Please use '0' for Internal, '1' for Recovered.
    Select From List By Value    ${clock_type_locator}    ${clock_type}
...
2 Mbps frame channels for Teleprotection
    [Arguments]    ${value}
    [Documentation]    This keyword configures the "2 Mbps frame channels for Teleprotection" checkbox in the IDTU section.
    ${input_text_locator}    Set Variable    xpath=//input[@name='dtproCfgInterfaceSlots']
    Scroll Element Into View Softly    ${input_text_locator}
    Wait Until Element Is Visible    ${input_text_locator}    timeout=5s
    Input Text By Pressing Keys    ${input_text_locator}    ${value}
# ******************************************************************************************************************************
#3rd Section
Command output blocking in case of BER alarm Checkbox
    ...
    ${is_in_range}    Validate Number In Range - int    ${value}    0    1
    Run Keyword If    not ${is_in_range}    Fail    Invalid value for Command output blocking in case of BER alarm. Please use '1' for checked, '0' for unchecked.
