...
#*************************************************************************************************************************
# 2nd Section
Local TX frequencies and remote reception times
    Reset TX Frequency Selection
    Autoconfig Apllication Type    1    0    ${Telep_number_to_config}    ${available_modules_commands}    # Application_operation_mode 0: Direct
    ${frequencies_list}    Open TX Frequencies and Show Them    1    ${Telep_number_to_config}    ${available_modules_commands}
    Frequency Selection    1110    ${frequencies_list}
    ${frequencies_list}    Open TX Frequencies and Show Them    2    ${Telep_number_to_config}    ${available_modules_commands}
    Frequency Selection    1590    ${frequencies_list}
    ${frequencies_list}    Open TX Frequencies and Show Them    3    ${Telep_number_to_config}    ${available_modules_commands}
    Frequency Selection    2070    ${frequencies_list}
    ${frequencies_list}    Open TX Frequencies and Show Them    4    ${Telep_number_to_config}    ${available_modules_commands}
    Frequency Selection    2550    ${frequencies_list}
Invalid TX Frequency Selection
    ${frequencies_list}    Open TX Frequencies and Show Them    4    ${Telep_number_to_config}    ${available_modules_commands}
    Frequency Selection    9902    ${frequencies_list}

Local RX frequencies and transmission times
    Reset RX Frequency Selection
    Autoconfig Apllication Type    0    2    ${Telep_number_to_config}    ${available_modules_commands}    # Application_operation_mode 2: Blocking
    ${frequencies_list}    Open RX Frequencies and Show Them    1    ${Telep_number_to_config}    ${available_modules_commands}
    Frequency Selection    870    ${frequencies_list}
    ${frequencies_list}    Open RX Frequencies and Show Them    2    ${Telep_number_to_config}    ${available_modules_commands}
    Frequency Selection    1650    ${frequencies_list}
Invalid RX Channel
    ${frequencies_list}    Open RX Frequencies and Show Them    3    ${Telep_number_to_config}    ${available_modules_commands}
    Frequency Selection    1650    ${frequencies_list}

Invalid RX Frequency Selection
    ${frequencies_list}    Open RX Frequencies and Show Them    2    ${Telep_number_to_config}    ${available_modules_commands}
    Frequency Selection    9902    ${frequencies_list}
...
PreCopy Tx To RX
    Autoconfig Apllication Type    1    1    ${Telep_number_to_config}    ${available_modules_commands}    # Application_operation_mode 1: Permissive
Copy Tx To Rx
...
Activation Threshold Assign for low Snr Ratio Alarm
    ...
    IF    ${is_in_range}
        ${Snr_Threshold_xpath}=    Set Variable    xpath=//input[contains(@name, 'dtproFskLowSnrThreshold')]
...
Deactivation Threshold Assign for low Snr Ratio Alarm
    ...
    IF    ${is_in_range}
        ${Snr_Threshold_xpath}=    Set Variable    xpath=//input[contains(@name, 'dtproFskHighSnrThreshold')]
        Clear Element Text    ${Snr_Threshold_xpath}
        Check If an Error Popup Appeared & Skip It
        Press Keys    ${Snr_Threshold_xpath}    ${Snr_Threshold}
        Press Key     ${Snr_Threshold_xpath}    \\09
...
# Convert Command Num To Letter
#     [Documentation]    Converts the Command Number to Letter Format. 1 -> A, 2 -> B, 3 -> C ...
#     [Arguments]    ${Command_Num}
#     IF    ${Command_Num} == 1
#         ${Command_Letter}=    Set Variable    A
#     ELSE IF    ${Command_Num} == 2
#         ${Command_Letter}=    Set Variable    B
#     ELSE IF    ${Command_Num} == 3
#         ${Command_Letter}=    Set Variable    C
#     ELSE IF    ${Command_Num} == 4
#         ${Command_Letter}=    Set Variable    D
#     ELSE IF    ${Command_Num} == 5
#         ${Command_Letter}=    Set Variable    E
#     ELSE IF    ${Command_Num} == 6
#         ${Command_Letter}=    Set Variable    F
#     ELSE IF    ${Command_Num} == 7
#         ${Command_Letter}=    Set Variable    G
#     ELSE IF    ${Command_Num} == 8
#         ${Command_Letter}=    Set Variable    H
#     ELSE IF    ${Command_Num} == 9
#         ${Command_Letter}=    Set Variable    I
#     ELSE IF    ${Command_Num} == 10
#         ${Command_Letter}=    Set Variable    J
#     ELSE IF    ${Command_Num} == 11
#         ${Command_Letter}=    Set Variable    K
#     ELSE IF    ${Command_Num} == 12
#         ${Command_Letter}=    Set Variable    L
#     ELSE IF    ${Command_Num} == 13
#         ${Command_Letter}=    Set Variable    M
#     ELSE IF    ${Command_Num} == 14
#         ${Command_Letter}=    Set Variable    N
#     ELSE IF    ${Command_Num} == 15
#         ${Command_Letter}=    Set Variable    O
#     ELSE IF    ${Command_Num} == 16
#         ${Command_Letter}=    Set Variable    P
#     END

#     Return From Keyword    ${Command_Letter}
...
Assign TX Application Type To Channel
    ...
    ${Application_box_xpath}=    Set Variable    xpath=//div[@class='infoInpSelect']/select[@var_name= 'dtproCommand${Channel}CfgTransmitTime']
...
Assign RX Application Type To Channel
    [Documentation]    Assign to the Command Number passed as an argument the Application Type Selected. ${Application_Operation_Mode} = 0 : Direct. ${Application_Operation_Mode} = 1 : Permissive. ${Application_Operation_Mode} = 2 : Blocking. 
    [Arguments]    ${Command}    ${Application_Operation_Mode}
    ${Application_box_xpath}=    Set Variable    xpath=//div[@class='infoInpSelect']/select[@var_name= 'dtproCommand${Command}CfgReceiveTime']
    Wait Until Element Is Visible    ${Application_box_xpath}    timeout=3s
    Select From List By Value    ${Application_box_xpath}    ${Application_Operation_Mode}
...
Autoconfig Apllication Type
    [Documentation]    It is recommended making a TX or RX Reset Frequency Selection BEFORE USING THIS KEYWORD. Autoconfig the same Application Operation Mode to all available channels. TX_RX_type is passed considering: '1': TX ; '0': RX. ${Application_Operation_Mode} = 0 : Direct. ${Application_Operation_Mode} = 1 : Permissive. ${Application_Operation_Mode} = 2 : Blocking. 
...
Reset TX Frequency Selection
    [Documentation]    Change the value of the 1st channel application input box to a different one from the actual. This way, all the frequencies will be reset to none.
    ${Channel1_application_xpath}=    Set Variable    xpath=//div[@class='infoInpSelect']/select[@var_name= 'dtproCommand1CfgTransmitTime']
    Wait Until Element Is Visible    ${Channel1_application_xpath}    timeout=5s
    ${current_label}=    Get Selected List Label    ${Channel1_application_xpath}
    ${current_value}=    Get Selected List Value    ${Channel1_application_xpath}
    ...
    # '0': Direct ; '1': Permissive ; '2': Blocking
    IF    ${current_value} == 0
    ...
    Assign TX Application Type To Channel    1    ${selected_value}



Reset RX Frequency Selection
    [Documentation]    Change the value of the 1st channel application input box to a different one from the actual. This way, all the frequencies will be reset to none.
    ${Channel1_application_xpath}=    Set Variable    xpath=//div[@class='infoInpSelect']/select[@var_name= 'dtproCommand1CfgReceiveTime']
    Wait Until Element Is Visible    ${Channel1_application_xpath}    timeout=5s
    ...
    Assign RX Application Type To Channel    1    ${selected_value}
...
Open TX Frequencies and Show Them
    ...
    [Arguments]    ${Channel}    ${Tp_num_id}    ${available_commands}
    ${index}=    Evaluate    ${Tp_num_id} - 1
    ${available_tx_commands}=    Set Variable    ${available_commands}[${index}]
    ${available_tx_commands}=    Evaluate    ${available_tx_commands}
    ${available_channel}    Evaluate    ${Channel} <= ${available_tx_commands}
    IF    ${available_channel}
        ${Channel}=    Evaluate    ${Channel} - 1
        Button    sFrT_${Channel}    Frequency selection
    ...
    ELSE
        Fail    Channel not available within the maximum TX TP commands.
    END
...
Open RX Frequencies and Show Them
    ...
    ${available_rx_commands}=    Evaluate    ${available_rx_commands}

    ${available_channel}    Evaluate    ${Channel} <= ${available_rx_commands}
    IF    ${available_channel}
        ${Channel}=    Evaluate    ${Channel} - 1
        Button    sFrR_${Channel}    Frequency selection
        ${frequencies_list}    Show and Return Available frequencies List
        # Log To Console    ${frequencies_list}
        Return From Keyword    ${frequencies_list}
    ELSE
        Fail    Channel not available within the maximum RX TP commands.
    END
