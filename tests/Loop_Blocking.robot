...
#************************************************************************************************************************
# LOOP & BLOCKING VARIABLES
# ${Activate_Desactivate}=1: Activate ; ${Activate_Desactivate}=0: Desactivate inputs.
# Available duration values: 0 (Permanently), 10(s), 30(s), 60(s), 600(s). 
# Available types: 0: NONE, 1: INTERNAL, 2: LINE

${LOOP_TELEPROTECTION_NUMBER}    1    
${ACTIVATE_DEACTIVATE_LOOP}    1
${LOOP_TYPE}    1
${LOOP_DURATION}    0

${BLOCKING_TELEPROTECTION_NUMBER}    1
${ACTIVATE_DEACTIVATE_BLOCKING}    1
${BLOCKING_DURATION}    0

*** Test Cases ***
Current Loop and Blocking State
    Setup Folder Section
    ${Loop_current_state_1}    Current Loop Activation_Desactivaton State    1
    ${Loop_current_state_2}    Current Loop Activation_Desactivaton State    2

    ${Blocking_current_state_1}    Current Blocking Activation_Desactivaton State    1
    ${Blocking_current_state_2}    Current Blocking Activation_Desactivaton State    2

    # Loop Current State
    Log    \${Loop_State_1} = ${Loop_current_state_1}[0]
    Log    \${Loop_State_2} = ${Loop_current_state_2}[0]
    # Loop Module Name
    Log    \${Loop_ModuleName_1} = ${Loop_current_state_1}[1]
    Log    \${Loop_ModuleName_2} = ${Loop_current_state_2}[1]
    # Loop Type
    Log    \${Loop_Type_1} = ${Loop_current_state_1}[2]
    Log    \${Loop_Type_2} = ${Loop_current_state_2}[2]
#*************************************************************************************
    # Blocking Current State
    Log    \${Blocking_State_1} = ${Blocking_current_state_1}
    Log    \${Blocking_State_2} = ${Blocking_current_state_2}
    # Blocking Module Name
    Log    \${Blocking_ModuleName_1} = ${Blocking_current_state_1}[1]
    Log    \${Blocking_ModuleName_2} = ${Blocking_current_state_2}[1]
    # Blocking Type
    Log    \${Blocking_Type_1} = ${Blocking_current_state_1}[2]
    Log    \${Blocking_Type_2} = ${Blocking_current_state_2}[2]
...
Program Teleprotection Loop
    Setup Folder Section
    Configure and Program Loop    ${ACTIVATE_DEACTIVATE_LOOP}    ${LOOP_TELEPROTECTION_NUMBER}    ${LOOP_TYPE}    ${LOOP_DURATION}

# *************************************************************************************************************************
# BLOCKING ACTIVATION TESTCASES:
Program Teleprotection Blocking
    Setup Folder Section
    Configure and Program Blocking    ${ACTIVATE_DEACTIVATE_BLOCKING}    ${ACTIVATE_DEACTIVATE_BLOCKING}    ${BLOCKING_DURATION}
...
Configure and Program Loop
    [Arguments]    ${Activate_Desactivate_Loop}    ${Teleprotection_Number}    ${Type}    ${Duration}
    ...
    ${tp_info}    Current Loop Activation_Desactivaton State    ${Teleprotection_Number}
    ${current_state}    Get From List    ${tp_info}    0
    IF    ${current_state} == 0 and ${Activate_Desactivate_Loop} == 0
        Fail    Loop is already desactivated!

    ELSE IF    ${current_state} == 0 and ${Activate_Desactivate_Loop}
        Assign Loop    ${Teleprotection_Number}    ${Type}    ${Duration}
        Program Loop    ${Activate_Desactivate_Loop}    ${Teleprotection_Number}
        Wait Until Progress Bar Reaches 100 V2

    ELSE IF    ${current_state} and ${Activate_Desactivate_Loop} == 0
        Program Loop    ${Activate_Desactivate_Loop}    ${Teleprotection_Number}
        Wait Until Progress Bar Reaches 100 V2

    ELSE IF    ${current_state} and ${Activate_Desactivate_Loop}
...
Current Loop Activation_Desactivaton State
    ...
    [Documentation]    ${Tp_info}[0]: ${current_state}=1 if there is an active loop in the teleprotecion passed as an arguent. Return ${current_state}=0 if there is no activate loop. ${Tp_info}[1]: Current Module Name.   ${Tp_info}[2]: Current Loop Type   
    Sleep    4s
    ...
    ${current_module_name_xpath}=    Set Variable    xpath=(//td[@class='tGrpLab' and text()='TELEPROTECTION -${Teleprotection_Number}-'][1]/ancestor-or-self::tr//td)[2]//div[@class='infoUsrTxtPart']
    ${current_type_xpath}=    Set Variable    xpath=(//td[@class='tGrpLab' and text()='TELEPROTECTION -${Teleprotection_Number}-'][1]/ancestor-or-self::tr//td)[3]//select[@var_name='dtproMgmtLoopType']
    ...
    #Current Module Name
    ${current_module_name}    Get Text    ${current_module_name_xpath}

    #Current Loop Type
    ${current_type}    Get Element Attribute    ${current_type_xpath}    value

    # Log To Console    Loop state in Teleprotection ${Teleprotection_Number} : ${current_state}
    Append To List    ${Tp_info}    ${current_state}
    Append To List    ${Tp_info}    ${current_module_name}
    Append To List    ${Tp_info}    ${current_type}
    Return From Keyword    ${Tp_info}
...
Assign Loop
    ...
    Select From List By Value    ${Type_xpath}    ${Type}
...
Configure and Program Blocking
    ...
    [Documentation]    ${Activate_Desactivate}=1: Activate     ${Activate_Desactivate}=0: Desactivate inputs. Available duration values: 0 (Permanently), 10(s), 30(s), 60(s), 600(s).
     # We ensure the number passed does not exceed 2 (maximum teleprotection modules we can install).
    ${tp_info}    Current Blocking Activation_Desactivaton State    ${Teleprotection_Number}
    ${current_state}    Get From List    ${tp_info}    0
...
Current Blocking Activation_Desactivaton State
    [Arguments]    ${Teleprotection_Number}
    [Documentation]    ${Tp_info}[0]: ${current_state}=1 if there is an active blocking in the teleprotecion passed as an arguent. Return ${current_state}=0 if there is no activate blocking.  ${Tp_info}[1]: Current Module Name.   ${Tp_info}[2]: Current Blocking Type   
    Sleep    4s
