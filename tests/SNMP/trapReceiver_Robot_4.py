...
class trapReceiver_Robot_4:
    ROBOT_LIBRARY_SCOPE = 'TEST SUITE'
    
    # Definimos los OIDs fijos que queremos guardar
    _specific_desired_oids = {
        # Timestamp y estado de notificación
        # 'DIMAT-TPU1C-MIB::tpu1cNotifyTimeSecsUtc',
        # 'DIMAT-TPU1C-MIB::tpu1cNotifyTimeMillisecs',
        'DIMAT-TPU1C-MIB::tpu1cNotifyState',
        # 'DIMAT-TPU1C-MIB::tpu1cNotifyIONum',
        # Inputs
        ...
        # Nombres de los OIDs para los componentes del timestamp
    ...
    def __init__(self):
        ...
        self._classification_oid_key_numeric = "1.3.6.1.6.3.1.1.4.1.0" # OID estándar que identifica el tipo de notificación en SNMPv2c y posterior
        self._classification_oid_key_named = "SNMPv2-MIB::snmpTrapOID.0" # Symbolic name
    ...
    def _callback(self, transportDispatcher, transportDomain, transportAddress, wholeMsg):
        try:
            while wholeMsg:
                ...
                processed_var_binds = [] # Lista de diccionarios con OID resuelto y valor
                temp_raw_value_store = {} # Almacena temporalmente todos los valores raw por OID resuelto
                ...
                for oid, val in varBinds:
                    try:
                        ...
                        temp_raw_value_store[oid_name_str] = value_str # Guardar para cálculo de timestamp
                    except Exception as e:
                        ...
                        temp_raw_value_store[oid_name_str] = value_str # Guardamos para el cálculo del timestamp
                
                 # Cálculo del Timestamp
                calculated_timestamp_iso = None
                # Obtenemos los valores de segundos y milisegundos de las definiciones de OID que nos proporcionan nuestras MIBs
                ...
                if secs_str is not None:
                    try:
                        secs_int = int(secs_str) #Aseguramos que el valor de segundos es un entero
                        dt_object = datetime.fromtimestamp(secs_int, tz=timezone.utc) # Hora teniendo en cuenta solo los segundos
                        
                        if millis_str is not None:
                            try:
                                millis_int = int(millis_str)
                                # Nos aseguramos que los milisegundos estén en el rango correcto
                                # y los microsegundos (0-999999)
                                if 0 <= millis_int <= 999:
                                     dt_object = dt_object + timedelta(milliseconds=millis_int) #Actualizamos el objeto datetime con los milisegundos
                                else:
                                    logger.warning(f"Valor de milisegundos '{millis_int}' fuera de rango (0-999) para OID {self._millis_oid_name}. Se ignorarán los milisegundos.")
                            except ValueError:
                                logger.warning(f"No se pudo convertir valor de milisegundos '{millis_str}' a entero para OID {self._millis_oid_name}. Se ignorarán los milisegundos.")
                        
                        calculated_timestamp_iso = dt_object.isoformat()
                    except ValueError:
                        logger.warning(f"No se pudo convertir valor de segundos '{secs_str}' a entero para OID {self._secs_oid_name}.")
                    except Exception as e_ts:
                        logger.error(f"Error calculando timestamp combinado: {e_ts}")
                
                # Fin Cálculo Timestamp 
                ...
                # Datos mínimos para almacenar
                trap_details_to_store = {
                    'timestamp_received_utc': datetime.now(timezone.utc).isoformat(), # Timestamp de recepción UTC DEL PROPIO TRAP SNMP
                ...
                with self._lock:
                    ...
                    if safe_event_key not in self._classified_traps:
                        self._classified_traps[safe_event_key] = collections.deque() # Declaramos una nueva cola para este tipo de evento.
                    self._classified_traps[safe_event_key].append(trap_details_to_store) # Añadimos el trap a la cola clasificada.
    ...
    def start_trap_listener(self, listen_ip="0.0.0.0", listen_port=162, mib_dirs_string=""):
        ...
        # Iniciar el hilo del dispatcher
        self._listener_thread = threading.Thread(target=self._run_dispatcher_loop, daemon=True)
        self._listener_thread.start()
        
        time.sleep(0.1)     # Nos aseguramos que el hilo se inicie correctamente
    ...
    def get_traps_by_event_type(self, event_type, consume_traps=True, limit=0):
        ...
        with self._lock:
            ...
            else: # Si no queremos consumir traps
