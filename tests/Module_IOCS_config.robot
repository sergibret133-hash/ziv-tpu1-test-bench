...
Check if Telep available and Open Section
    ...
    ${Telep_name_to_config}=    Set Variable    IOCS
...
#     Autoconfig Tx_commands    ${Telep_number_to_config}    ${available_modules_commands}    Blocking
#     Sleep    2s

#     Autoconfig Tx_commands    ${Telep_number_to_config}    ${available_modules_commands}    Permissive trip
#     Sleep    2s

#     Autoconfig Tx_commands    ${Telep_number_to_config}    ${available_modules_commands}    Direct trip
#     Sleep    2s
# Programm
#     Program Old    sCOM
#*************************************************************************************************************************
...
#     ${with_tx_time_measure}    Set Variable    1
#     With TX time measure Checkbox    ${with_tx_time_measure}
#     Sleep    2s
...
Assign Radio IOCS Button
    [Arguments]    ${command_number}    ${radio_name}
    [Documentation]    Assigns a radio button to a command in the IOCS section. Available Command Type configurations: Blocking, Permissive trip and Direct trip     
    ${radio_type}    Validate Radio Name    ${radio_name}
    ${locator_base}=    Set Variable    //div[contains(@class, 'infoDatInpLab_content') and contains(text(), '${radio_name}')]
    ${locator_xpath}=    Set Variable    xpath=(${locator_base}/ancestor::tr//input[contains(@type,'radio')])[${command_number}]
    # Scroll to the radio button element
    Scroll Element Into View Softly    ${locator_xpath}
    # Wait for the radio button to be visible
    Wait Until Element Is Visible    ${locator_xpath}    timeout=5s
    # Select the radio button
    Click Element    ${locator_xpath}

Validate Radio Name
    [Arguments]    ${radio_name}
    [Documentation]    Validates the radio name to ensure it is one of the expected values.
    IF    "${radio_name}" == "Blocking"
        ${radio_type}    Set Variable    0
    ELSE IF    "${radio_name}" == "Permissive trip"
        ${radio_type}    Set Variable    1
    ELSE IF    "${radio_name}" == "Direct trip"
...
Autoconfig Tx_commands
    ...
    FOR    ${command}    IN RANGE    1    ${Max_Tx_Commands}
        Assign Radio IOCS Button    ${command}    ${Command Type}
    END

# ******************************************************************************************************************************
# 2nd Section
Identifiers config
    [Arguments]    ${TX ID}    ${RX ID}
    [Documentation]    This keyword configures the identifiers for the Tx and Rx commands in the IOCS section. It requires the Tx and Rx IDs as arguments.
    ${TX_ID_locator}    Set Variable    xpath=//input[@name='dtproCfgTransmitIdCode_ext']
    ${RX_ID_locator}    Set Variable    xpath=//input[@name='dtproCfgReceiveIdCode_ext']
    Scroll Element Into View Softly    ${TX_ID_locator}
    Wait Until Element Is Visible    ${TX_ID_locator}    timeout=5s
...
With TX time measure Checkbox
    [Arguments]    ${value}
    [Documentation]    This keyword configures the "With TX time measure" checkbox in the IOCS section. It requires the value as an argument (1 for checked, 0 for unchecked).
...
Clock Type Selection
    ...
    [Documentation]    Clock Type names available: Internal/Recovered. This keyword configures the clock type in the IOCS section. It requires the clock type as an argument.
...
Validate Clock Type
    [Arguments]    ${clock_type}
    [Documentation]    Validates the clock type in the IOCS section. It requires the clock type as an argument. Only Internal and Recovered are valid clock types.
...
Command output blocking in case of BER alarm Checkbox
    [Arguments]    ${value}
    [Documentation]    This keyword configures the "Command output blocking in case of BER alarm" checkbox in the IOCS section. It requires the value as an argument (1 for checked, 0 for unchecked).
...
ACTIVATION threshold for BER alarm Selection
    ...
    [Documentation]    This keyword configures the activation threshold for the BER alarm in the IOCS section. It requires the activation threshold as an argument. Avaliable values: 1.0E-2, 0.5E-2, 1.0E-3, 0.5E-3, 1.0E-4, 0.5E-4, 1.0E-5, 0.5E-5, 1.0E-6, 0.5E-6, 1.0E-7, 0.5E-7, 1.0E-8, 0.5E-8, 1.0E-9, 0.5E-9
...
DEACTIVATION threshold for BER alarm Selection
    ...
    [Documentation]    This keyword configures the deactivation threshold for the BER alarm in the IOCS section. It requires the deactivation threshold as an argument. Avaliable values: 1.0E-2, 0.5E-2, 1.0E-3, 0.5E-3, 1.0E-4, 0.5E-4, 1.0E-5, 0.5E-5, 1.0E-6, 0.5E-6, 1.0E-7, 0.5E-7, 1.0E-8, 0.5E-8, 1.0E-9, 0.5E-9
...
Validate Activation_Deactivation Threshold
    [Arguments]    ${activation_threshold}
    [Documentation]    Validates the activation threshold for the BER alarm in the IOCS section. It requires the activation threshold as an argument. Avaliable values: 1.0E-2, 0.5E-2, 1.0E-3, 0.5E-3, 1.0E-4, 0.5E-4, 1.0E-5, 0.5E-5, 1.0E-6, 0.5E-6, 1.0E-7, 0.5E-7, 1.0E-8, 0.5E-8, 1.0E-9, 0.5E-9
